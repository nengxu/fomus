...
(defparameter +notemark-type+
  '(or*
    ...
    (let* ((x (member :textnote :texttempo :textdyn :text)))
      (or* (list* x string) (list* x string (member :up :down :nopos)) (list* x (member :up :down :nopos) string)))
    ...
    (let* ((x (member :starttext-)))
      (cons* x (or* (unique* tx 1 (list* string))
		    (unique* tx 1 (list* string (member :up :down :nopos)))
		    (unique* tx 1 (list* (member :up :down :nopos) string))
		    (list* string (unique* tx (integer 1)))
		    (list* (unique* tx (integer 1)) string)
		    (list* (member :up :down :nopos) string (unique* tx (integer 1)))
		    (list* (member :up :down :nopos) (unique* tx (integer 1)) string)
		    (list* string (member :up :down :nopos) (unique* tx (integer 1)))
		    (list* (unique* tx (integer 1)) (member :up :down :nopos) string)
		    (list* string (unique* tx (integer 1)) (member :up :down :nopos))
		    (list* (unique* tx (integer 1)) string (member :up :down :nopos)))))))
