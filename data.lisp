...
(defparameter +settings+
  `((:debug-filename (or null string)) (:verbose (integer 0 2)) 
    ...
    (:split-mod symbol) (:auto-accs-mod symbol) (:auto-voices-mod symbol) (:auto-distr-rests-mod symbol)
...
(defparameter +notemark-type+
  '(or*
    ...
    (let* ((x (member :textnote :texttempo :textdyn :text)))
      (or* (list* x string) (list* x string (member :up :down)) (list* x (member :up :down) string)))
    ...
    (let* ((x (member :starttext-)))
      (cons* x (or* (unique* tx 1 (list* string))
		    (unique* tx 1 (list* string (member :up :down)))
		    (unique* tx 1 (list* (member :up :down) string))
		    (list* string (unique* tx (integer 1)))
		    (list* (unique* tx (integer 1)) string)
		    (list* (member :up :down) string (unique* tx (integer 1)))
		    (list* (member :up :down) (unique* tx (integer 1)) string)
		    (list* string (member :up :down) (unique* tx (integer 1)))
		    (list* (unique* tx (integer 1)) (member :up :down) string)
		    (list* string (unique* tx (integer 1)) (member :up :down))
		    (list* (unique* tx (integer 1)) string (member :up :down)))))))
